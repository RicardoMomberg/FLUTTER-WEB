...
void main() {
  test('1 - Valida se o arquivo possui extensão .jpeg', () {
    final fileName = 'image.jpeg';
    final isValid = FileValidator.isJpeg(fileName);
    expect(isValid, true);
  });

  test('2 - Valida se o arquivo possui extensão .png', () {
  ...
  test('Retorna false para arquivos maiores que 5 MB', () {
    final fileBytes = Uint8List(FileValidator.maxFileSize + 1);
    final isValid = FileValidator.isFileSizeValid(fileBytes);
    expect(isValid, false);
  });

  test('1 - Valida a função completa de validação de arquivo', () {
    final fileBytes = Uint8List(FileValidator.maxFileSize + 1);
    final fileName = 'image.jpeg';
    final isValid = FileValidator.isFileSizeValid(fileBytes, fileName);
    expect(isValid, false);
  });

  test('2 - Valida a função completa de validação de arquivo', () {
    final fileBytes = Uint8List(FileValidator.maxFileSize);
    final fileName = 'image.jpeg';
    final isValid = FileValidator.isFileSizeValid(fileBytes, fileName);
    expect(isValid, true);
